...
public class Player : MonoBehaviour, IDamageable
{
    ...
    private long FinalAttackDamage()
    {
        if (IsCritical())
        {
            return (long)(AtkDamage.Value + (AtkDamage.Value * CritDamage.GetFloat()) * (1 + EquipStat / 100) * (1 + RetentionEffect / 100));
        }
        else
        {
            return (long)(AtkDamage.Value + (1 + EquipStat / 100) * (1 + RetentionEffect / 100));
        }
    }
